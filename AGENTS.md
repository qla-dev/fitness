...
- The local implementation is `src/services/local/`: `database.ts`, `localApi.ts` (the request router), and the `foodRepository` / `healthRepository` / `workoutRepository` modules. `localApi.ts` is the list of endpoints that actually exist; anything missing there is unimplemented locally.
- `database.ts` is **not** SQLite, whatever `expo-sqlite` being a dependency suggests: the whole database is one JSON value in AsyncStorage, read through `localTransaction`, which serializes every request behind a single queue. The consequence to design around is that there are no indexes and no partial reads — a repository that filters with `Array.find` over a table pays for the whole table, on every request. Narrow by date **before** parsing or mapping rows (`localMeasurements` and `localSessions` both take a range for this reason), and build a `Map` once rather than scanning per record inside a loop, as `importHealthData` does for its `health_key` lookups.
- `localTransaction` keeps the parsed database in memory between calls and writes back only when something changed: storage is read and Zod-validated once per launch, not once per request. A read skips serialization entirely, so any code path that mutates outside `saveRecord` / `deleteRecord` must either run under a non-GET request or call `markLocalDatabaseDirty()` — the seeding in `localApi.ts` does the latter. A failed operation or write drops the cache so the next transaction re-reads storage. Tests get a fresh copy from the global `beforeEach` in `jest.setup.js`; anything clearing AsyncStorage by hand must reset it too.
- The `changes` changelog is a capped debugging trail (`CHANGE_LOG_LIMIT`), not a sync journal — nothing reads it. Large bodies and results are summarized rather than stored, because a health import otherwise kept a second copy of everything it imported and every later request paid to parse it.
- Adding a data capability means adding a handler in `localApi.ts` plus repository support — never a server route.
- Health sync is **not** gated on `isLocalDataMode()`. Apple Health / Health Connect is the data source and the on-device database is the destination, so sync-on-open, background sync, and the iOS HealthKit observers all run in local mode. `resolveSyncConfigId()` in `storage.ts` supplies the synthetic `'local'` id (`LOCAL_SYNC_CONFIG_ID`) wherever a server config id would otherwise be required — sync-on-open's cooldown key and the backfill's checkpoint and pinned-server checks. Anything keyed on `getActiveServerConfig()` instead reads local mode as "no server" and silently does nothing.
...
- Tabs are `Dashboard`, `Diary`, `Add`, `Library`, and `Exercises`. `Add` is a center action in `CustomTabBar`, not a content screen. The `Exercises` tab mounts the same `ExercisesLibraryScreen` the Library drills into; the screen reads `navigation.canGoBack()` to tell the two apart, showing a back button only in the pushed copy and the profile button only at the tab root.
- The Exercises tab is a program store above the exercise library. `ProgramStore` (`src/components/ProgramStore.tsx`) renders the category chips, the featured card and the program rows as the list header when the search box is empty; tapping a row pushes `ExerciseProgram`, a product page styled like an app listing (`ExerciseProgramScreen`). The catalogue is `src/constants/exercisePrograms.ts` — 21 programs, each with at least 20 exercises across its sessions plus a nutrition block, typed by `src/types/exerciseProgram.ts`.
- Workout presets are called **Programs** in the UI. The API, types, routes and query keys all still say `preset` — only user-facing strings were renamed, so a `t()` default and its EN catalog entry read "program" while `workoutPresetsApi`, `WorkoutPresetDetail` and `workoutPresetsQueryKey` keep their names. The one exception is `workoutComplete.confirm.updatePresetMessage`, whose `{{preset}}` placeholder keeps its original name: renaming an interpolation variable invalidates every existing translation of the string.
- Start on a store program opens `ProgramPurchaseScreen` (a native confirmation modal using `PromptScreen`, nothing is charged), which calls `installProgramAsPresets` (`src/services/programToPresets.ts`) to create one preset containing all sessions in order, with session names preserved in set notes. Its success view reuses `SuccessHero` from workout completion and offers Open program and My Programs actions. The preset schema needs a real `exercise_id`, so every movement name is resolved against the library and then the provider (importing the match); resolution is cached per distinct name because programs repeat movements across sessions, unresolvable movements are reported in the result rather than dropped silently, and a program that resolves nothing creates no preset.
- Program copy is content, not chrome: names, taglines, coaching notes and nutrition text stay literal in the data file the way provider exercise names do, and only the surrounding labels go through `t()`. A session's name is stored under `name` rather than `title` because the i18n audit treats a `title` string literal as untranslated UI text wherever it appears.
- Searching the Exercises screen hides the store and merges two halves into one list: matching saved exercises under a 'My exercises' heading, then provider results under 'Online' (deduplicated by name). Tapping an online row imports it through `importExercise` and opens the saved copy. The provider is the user's first active exercise provider — there is no picker, unlike `ExerciseSearchScreen`.
- Profile is no longer a tab: `SettingsScreen` is the root-stack `Profile` route, reached from the profile button in the top-right of every tab header. Anything that used to send users to the Settings tab pushes `Profile` instead.
- `TabHeader` (`src/components/TabHeader.tsx`) is the shared tab bar chrome on the fallback header path: the day on the left, the screen name centered, and the profile button in the top-right. It is presentational — the profile press is passed in as `onProfilePress`, not resolved from navigation context, so screens can be rendered in tests without a container. Tabs pass no `onPreviousDay`/`onNextDay`, because their content is swipeable; `FamilyDiaryScreen` passes them because it has no swipe gesture and the picker would otherwise be its only way to change days.
...
- `TabsLayout` switches at runtime via `useNativeIOSTabsActive()` (`services/nativeTabBarPreference.ts`): native tabs require iOS 26+ Liquid Glass support AND the user's `liquidGlassTabBarEnabled` preference (on by default where Liquid Glass is available; a v1→v2 store migration switched it on once for existing iOS 26+ users). Everything else renders `CustomTabBar` (`TAB_BAR_HEIGHT = 56`); `ActiveWorkoutBar` reads the native tab-bar height instead when native tabs are active.
...
- Back buttons are the chevron alone: every root-stack and tab-local screen sets `headerBackButtonDisplayMode: 'minimal'` in `App.tsx` / `TabsLayout.tsx` rather than `headerBackTitle`, so iOS never prints the previous screen's name. Close/cancel modal headers need neither option.
...
- Two stored sync windows, and only one surface uses the wider one. **History Sync Range** (`timeRange`, `DEFAULT_HISTORY_SYNC_RANGE` = `365d`) belongs to the "Sync History Now" button on `AppleHealthCheckScreen` alone; it is held in screen state, opens at a full year every visit and is deliberately never loaded back from storage, so narrowing it applies to that visit only. **Startup Sync Range** (`dailySyncRange`, `DEFAULT_DAILY_SYNC_RANGE` = `3d`) is what everything else runs on: sync-on-open, foreground-return — including the Apple Health screen's own re-sync when the user comes back from the Health app — background sync, the Dashboard activity card and the AddSheet row, all through `prepareManualHealthSync`. The metric readout on that screen reads the startup window too, since `fetchHealthDisplayData` pulls raw records per metric and a year of them is a screen-load cost. A new sync entry point takes the startup window unless it is a deliberate catch-up press.
...
- **Inbound HKWorkouts are the exception: they are guarded by metadata, not by bundle id.** HealthKit files the qla.fit watch app's own recordings under the *phone* app's bundle id, so `isOwnRecord` could not tell a writeback echo from a workout the watch recorded — and dropped both, which meant a session started on the watch never reached the diary and the day showed only its Apple Exercise Time total. `isAlreadyLoggedWorkout` in `healthkit/dataTransformation.ts` keys on metadata instead: `QlaFitWritebackVersion` (plus the pre-rebrand `SparkyWritebackVersion`, which older saves still carry) marks writeback's own saves, and `QlaFitWatchOrigin: 'phone'` marks a watch workout the phone started — the phone's recorder is already saving that session, so importing the watch's copy would log the effort twice. A workout with no metadata is another app's and imports. The keys live in `healthkit/writebackMappers.ts` and are duplicated by hand in `targets/watch/WorkoutManager.swift`, like the WatchConnectivity message keys. This only works because `handleWorkout` forwards the sample's whole `metadata`; rebuilding that field from the flattened timezone alone silently disables the guard.
- Imported workouts are named from `HKWorkoutBrandName` when they carry one, falling back to `ACTIVITY_MAP`. The watch stamps the sport that was picked, so a CrossFit session reads "CrossFit" rather than Apple's "Cross Training" — only the numeric activity type crosses into HealthKit, and its enum name is Apple's wording, not the user's. Provider names stay literal, like every other piece of provider content.
- The health importer files two Apple day totals as exercise entries: `Active Calories` and `Apple Exercise Time` (`healthRepository.ts`). They are rings, not efforts — no clock time, and exercise minutes carry no calories — so anything listing what someone *did* filters them with `isProviderDayTotal` (`utils/workoutSession.ts`); `ActivityHistoryScreen` and the hourly chart both do. Their figures still reach the dashboard and the exercise stats, which read them deliberately.
...
- Progress photos are split by what you are doing: **recording a day happens on the tracker, looking back happens on the screen.** `PhotoDayCapture` (`src/components/PhotoDayCapture.tsx`) sits in `DiaryScreen` under the meals and owns the day's three angles, the picker, the upload and the remove; `ProgressPhotosScreen` is the history of one angle and nothing else. They used to render the same `PhotoDaySlots` block, so the tracker's slots could only push to a second copy of themselves — two taps and a transition to reach a picker already under the thumb. The screen keeps a timeline of that angle's recent shoots, each row carrying its weight and the delta against the previous one - a preview rather than the archive, see the History note below. `ProgressPhotoCompareScreen` (two days side by side) and `ProgressPhotoTimelapseScreen` (cross-faded playback) hang off it. Reached from the `AddSheet` row and `ProgressPhotosCard` on the Dashboard. `ProgressPhotoViewer` is the full-screen pinch-zoom viewer - deliberately not `ImageLightbox`, which resolves URIs through the food image source context and takes plain string URLs.
...
- Adding and removing write immediately rather than staging behind a Save: this sits inside a day you are browsing, and unsaved state plus a back-guard does not belong there. One pick is one request, so uploads stay serial anyway. Replacing needs no delete first because the server upserts on `(user_id, entry_date, photo_type)`; removal confirms first.
- `PhotoDaySlots` shows all three angles at once so a day's gaps read at a glance. Camera-or-library is the system's own menu (`MenuView` from `@expo/ui/community/menu`, the way `WaterTile`'s actions are) rather than an app-drawn sheet: it is two choices on the way to a native picker, and a sheet in front of a system picker is two modals for one decision. The empty placeholder opens it on a **single tap** — `shouldOpenOnLongPress` stays off — because an empty slot has nothing else a tap could mean. Viewing and managing stay separate targets on a filled slot: the photo opens the viewer, the corner button opens the same menu plus Remove. A MenuView's trigger is a native host that sizes itself to its content, so both triggers get explicit pixel dimensions (the row is measured once and divided) — a percentage has nothing to resolve against and the tap target collapses onto whatever is inside it, the same reason `WaterTile` hands its menu a measured width.
...
- One angle concept per surface, which is what splitting the two fixed: the tracker's `PhotoDayCapture` is angle-agnostic and always shows all three, while the screen's `SegmentedControl` under the History heading scopes the timeline and the Compare / Time-lapse params. The screen no longer carries day slots at all, so its angle names appear once.
...
- Measurement tiles have one source of truth: `utils/measurementFields.ts` (label, unit, format, input parsing and storage conversion per field), `components/measurementTiles.tsx` (the tile model and card), `useMeasurementHistory` (one range read giving each field its last recorded value and yesterday's, for the trend chip and the not-recorded-today fallback). `MeasurementsSummary` shows Weight and Body fat and nothing else — not the other fields that happen to hold a value, and not the custom entries — because it sits above the meal list and its height has to be the same every day; it passes `restrictTo: ALWAYS_SHOWN_FIELDS` for that. `MoreMeasurementsSheet` shows every field; tapping either opens `MeasurementRecordSheet` for that one field. Adding a check-in field means adding it to the registry, not to three lists. Both of a tile's top corners always say something, because an empty corner reads as a tile still loading beside siblings that have finished. Top-left is a three-way note: "Not recorded today" when the day holds no value for the field (`recordedToday`, true whether it is falling back to an older reading or has never been filled), else "Previous: <value>" from `previousValue` — the same day-before reading the trend chip measures against, so the two can never disagree — else "No history data". Top-right is the trend chip, and a field with no reading at all still reports the flat rule rather than rendering nothing.
- `PROFILE_FIELDS` (currently just Height) are standing facts about the person, so `buildMeasurementTiles` leaves them out of the daily tiles and `ProfileStats` shows them beside Age on the Profile card, above `MyLibrarySection`. They are still stored on the check-in like any other measurement, and `ProfileStats` edits Height through the same `MeasurementRecordSheet`; only where they are shown has changed. Age is read-only — it is derived from `profile.date_of_birth` and there is no date-of-birth editor. The summary sits **above** the meal cards in the diary, with `CheckInPhotosSummary` under it.
- Only one bottom sheet is on screen at a time: a sheet that leads to another dismisses itself first and hands the choice back through its `onClose`. Stacked `BottomSheetModal`s flash and fight for the same gesture. Inside a sheet, scrollable content is `BottomSheetScrollView` (a plain `ScrollView` measures as collapsed under dynamic sizing), text inputs are `BottomSheetTextInput` (via `InputComponent` on `PillInput`/`StepperInput`), and cards are `bg-raised` with **no** border, because the sheet itself is `--color-surface` and the raised fill already separates them — an outline on top of that just draws a box around every card. Do not put a `maxHeight` on the scroll view either: the sheet caps itself at `topInset` and scrolls past it, so a fixed cap only reserves empty space below short content.
...
- Themes are Light, Dark, and System. There is exactly one dark appearance: the Uniwind `dark` theme carries the true-black AMOLED palette, so the OS dark appearance under System renders the same colors as picking Dark by hand, and there is no separate `amoled` Uniwind theme. `themeService.ts` owns persistence and folds a legacy stored `Amoled` preference into `Dark`; `App.tsx` syncs Android navigation bar style.
...
- A `SettingsRow` subtitle stays on one line and truncates with a trailing ellipsis, so a row's height never depends on how long its label happens to be. Rows whose subtitle is a full explanation rather than a value (the switch rows in App/Notification/Workout settings) opt out with `subtitleNumberOfLines={0}`.
- A row that opens a choice opens a screen, not a sheet: `ProfileGoals` lists every goal and `ProfileEdit` edits one value, and `ProfileTheme` lists the appearance options. Keep new profile rows on that pattern.
