...
- Start on a store program opens `ProgramPurchaseSheet` (a confirm-to-add sheet, nothing is charged), which calls `installProgramAsPresets` (`src/services/programToPresets.ts`) to create one preset per session. The preset schema needs a real `exercise_id`, so every movement name is resolved against the library and then the provider (importing the match); resolution is cached per distinct name because programs repeat movements across sessions, unresolvable movements are reported in the result rather than dropped silently, and a session that resolves nothing creates no preset.
