...
export function localTransaction<T>(
  ...
  const task = tail.then(async () => {
    ...
    const db = raw === null ? initialDatabase() : databaseSchema.parse(JSON.parse(raw));
    const result = operation(db);
    if (mutation) {
      db.revision += 1;
      db.changes.push({
        id: randomUUID(), revision: db.revision,
        timestamp: new Date().toISOString(), ...mutation, result,
      });
    }
...
export const asRecords = (value: unknown): LocalRecord[] => z.array(record).parse(value ?? []);
export const findRecord = (db: LocalDatabase, name: string, id: unknown): LocalRecord => {
  const row = table(db, name).find((item) => String(item.id) === String(id));
  if (!row) throw new Error(`Local ${name} record not found: ${String(id)}`);
  return row;
};
export function saveRecord(db: LocalDatabase, name: string, body: LocalRecord, id?: unknown): LocalRecord {
  const rows = table(db, name);
  const previous = id === undefined ? undefined : findRecord(db, name, id);
  const now = new Date().toISOString();
  const row = { id: newId(), user_id: db.userId, created_at: now, ...previous, ...body, updated_at: now };
  if (previous) rows[rows.indexOf(previous)] = row;
  else rows.push(row);
  return row;
}
export function deleteRecord(db: LocalDatabase, name: string, id: unknown): void {
