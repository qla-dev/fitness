...
describe('buildHourlyExerciseMinutes', () => {
  ...
  test('falls back to created_at when no entry_time was recorded', () => {
    const createdAt = new Date(2024, 5, 15, 14, 15).toISOString();
    const hours = buildHourlyExerciseMinutes([
      individual({ created_at: createdAt, duration_minutes: 30 }),
    ]);
